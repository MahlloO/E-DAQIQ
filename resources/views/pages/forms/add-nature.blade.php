        ...
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                        <button name="add" class="btn btn-primary submit mt-2 mb-4" onclick="duplicateInputs()">إضافة ملف الاختصاص القضائي اخر </button>

                        <form method="post" action="{{url('/add-nature')}}">
                        @method("post")
                        @csrf
                            <div class="input-container">
                                ...
                                <div class="row mx-auto m-lg-3 mb-5" >
                                    <div class="col-sm-4">
                                        <div class="mb-3">
                                            <label class="form-label"><span class="text-danger"> *</span> رقم ملف الاختصاص القضائي</label>
                                            <input type="number" class="form-control"  id="code" name="code[]"   value="{{ (old('code')!== null)?old('code'):(isset($oldValues)?$oldValues['code']:'') }}"   maxlength="5" required>
                                        </div>
                                    </div>
                                    <div class="col-sm-4">
                                        <div class="mb-3">
                                            <label class="form-label"><span class="text-danger"> *</span> نوع ملف الاختصاص القضائي</label>
                                            <input type="text" class="form-control"  id="typeDossier" name="typeDossier[]"   value="{{ (old('typeDossier')!== null)?old('typeDossier'):(isset($oldValues)?$oldValues['typeDossier']:'') }}"   required>
                                        </div>
                                    </div>
                                    <div class="col-sm-4">
                                        <div class="mb-3">
                                            <label class="form-label"><span class="text-danger"> *</span> نوع ملف الاختصاص القضائيم</label>
                                            <select class="form-select form-select-lg mb-2"   name="typeTibunale[]"  id="mySelect">
                                                    <option value="المحكمة الإبتدائية" {{ (isset($oldValues) and $oldValues['typeTibunale']  ==  "المحكمة الإبتدائية"  ) ? 'selected' : '' }}>المحكمة الإبتدائية</option>
                                                    <option value="محكمة الإستئناف" {{ (isset($oldValues) and $oldValues['typeTibunale']  ==  "محكمة الإستئناف"  ) ? 'selected' : '' }}>محكمة الإستئناف</option>
                                                    <option value="المحكمة التجارية" {{ (isset($oldValues) and $oldValues['typeTibunale']  ==  "المحكمة التجارية"  ) ? 'selected' : '' }}>المحكمة التجارية</option>
                                                    <option value="المحكمة الإدارية" {{ (isset($oldValues) and $oldValues['typeTibunale']  ==  "المحكمة الإدارية"  ) ? 'selected' : '' }}>المحكمة الإدارية</option>
                                            </select>
                                        </div>
                                    </div>
