            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                        ...
                        <form method="post" action="{{url('/edit-nature',$nature->id)}}">
                        @method("put")
                        @csrf
                                ...
                                <div class="row mx-auto m-lg-3 mb-5" >
                                    <div class="col-sm-4">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" class="form-control"  id="code" name="code"   value="{{ (old('code')!== null)?old('code'):(isset($oldValues)?$oldValues['code']:$nature->codeTypeDossier) }}"   maxlength="5" required>
                                        </div>
                                    </div>
                                    <div class="col-sm-4">
                                        <div class="mb-3">
                                            <label class="form-label"><span class="text-danger"> *</span> نوع ملف الاختصاص القضائي</label>
                                            <input type="text" class="form-control"  id="typeDossier" name="typeDossier"   value="{{ (old('typeDossier')!== null)?old('typeDossier'):(isset($oldValues)?$oldValues['typeDossier']:$nature->typeDossier) }}"   required>
                                        </div>
                                    </div>
                                    <div class="col-sm-4">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select form-select-lg mb-2"   name="typeTibunale"  id="mySelect">
                                                    <option value="المحكمة الإبتدائية" {{ ($nature->typeTibunale ==  "المحكمة الإبتدائية"  ) ? 'selected' : '' }}>المحكمة الإبتدائية</option>
                                                    <option value="محكمة الإستئناف" {{ ($nature->typeTibunale ==  "محكمة الإستئناف"  ) ? 'selected' : '' }}>محكمة الإستئناف</option>
                                                    <option value="المحكمة التجارية" {{ ($nature->typeTibunale ==  "المحكمة التجارية"  ) ? 'selected' : '' }}>المحكمة التجارية</option>
                                                    <option value="المحكمة الإدارية" {{ ($nature->typeTibunale  ==  "المحكمة الإدارية"  ) ? 'selected' : '' }}>المحكمة الإدارية</option>
                                            </select>
                                        </div>
                                    </div>
