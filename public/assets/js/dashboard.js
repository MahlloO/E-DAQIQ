...
$(function() {
  ...
  // Fetch the monthly cases data from the controller
    $(document).ready(function() {
        $.ajax({
            ...
            success: function(response) {
                try {
                    var monthlyCases = response.map(function(data) {
                        return {
                            x: new Date(data.month + '-01'),
                            y: data.case_count
                        };
                    });

                    // Add an empty entry for each month that does not have a case
                    for (var i = 0; i < 12; i++) {
                        var month = new Date(2023, i, 1);
                        if (!monthlyCases.some(function(data) { return data.x === month; })) {
                            monthlyCases.push({
                                x: month,
                                y: 0
                            });
        ...
        $.ajax({
            ...
            success: function(response) {
                try {
                    ...
                    var apexBarChart = new ApexCharts(document.querySelector("#monthlySalesChartRTL1"), options);
                    apexBarChart.render();
                } catch (error) {
                    console.log("Error parsing JSON:", error);
                }
  ...
  if ($('#storageChart').length) {
      ...
      success: function(data) {
          // Parse the user count into the series array
          var series = [data];
